...
from Scheduler.Models.Services import ScheduleService
...
class TestScheduleService():
    ...
    def setUp(self):
        self.test_project = ProjectMixin(events=copy.copy(mock_data.m1_events), activities=copy.copy(mock_data.m1_activities))
        self.test_project_no_source = ProjectMixin(events=copy.copy(mock_data.no_source_events), activities=copy.copy(mock_data.no_source_activities))
        self.test_project_unorderable = ProjectMixin(events=copy.copy(mock_data.unorderable_events), activities=copy.copy(mock_data.unorderable_activities))
    ...
    def worker_assignments_single_worker(self):
        ...
        result = self.test_project.schedule(1) # 1 worker
    ...
    def worker_assignments_two_workers(self):
        ...
        result = self.test_project.schedule(2) # 2 workers
